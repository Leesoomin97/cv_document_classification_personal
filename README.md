...
|------|-----------|----------------|--------------|----------------|-----------------|
| **초반** | Ver.1–3 | Swin-Tiny 기반, 전처리만 변경하며 시도 | 성능 0.8 중반에서 정체 | EfficientNet(B3·B4·B5) 계열로 전환 | 백본 구조 선택의 중요성 인식 |
| **혼란기** | Ver.4–9 | EfficientNet 앙상블 → OCR·ViT 융합 시도 | OCR/ViT 추가 직후 0.1대 점수 폭락, GPU 세션 중단 | OCR 완전 제거, Vision-only로 회귀 | 앙상블이 항상 이득은 아님 |
| **전환점** | Ver.10–11 | **전처리 방향 재정리:** 기하학 중심 → **재질·노이즈 중심(Gaussian Blur, White Balance, CLAHE 등)** | 전처리 후 증강 적용하여 노이즈 심화 | 오프라인 증강 → 전처리 → 학습 순으로 재구성 | 데이터 품질과 순서가 모델보다 중요 |
| **성장기** | Ver.12–13 | B4 메인 + B5 서브 앙상블, 클래스 불균형 보정, 3·7·14 별도 보정 | Semantic 그룹 분류는 오히려 성능 저하 | 17개 클래스 그대로 학습 + 오버샘플링 | 개인 최고점 **0.9318** 달성 |
| **최종단계** | Ver.14 | B4 seed 앙상블 + B5 서브 조합, TTA 튜닝 | 과도한 TTA로 시간 증가 + 점수 출렁임 | TTA 강도 조절 및 최적 조합 확립 | 0.9 초반대 안정된 성능 |
...
## 🟣 `eda_preprocessing`, 'modeling' --- 실험 및 연구 브랜치 (Experimental)
